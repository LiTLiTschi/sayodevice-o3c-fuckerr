"""Control screen — device control (set pos, color, arg0, etc.)."""

from __future__ import annotations

from textual.app import ComposeResult
from textual.binding import Binding
from textual.screen import Screen
from textual.widgets import Footer, Header, Static, Input, Button, RichLog
from textual.containers import Vertical, Horizontal
from textual import work


class ControlScreen(Screen):
    """Device control panel for setting position, color, arg0."""

    BINDINGS = [
        Binding("escape", "app.switch_screen('dashboard')", "Back", show=True),
    ]

    DEFAULT_CSS = """
    ControlScreen {
        layout: vertical;
        overflow-y: auto;
    }
    #control-title {
        text-align: center;
        padding: 1 0 0 0;
        text-style: bold;
        color: $accent;
    }
    .section-title {
        padding: 0 2;
        text-style: bold;
        color: $accent;
    }
    .section-box {
        border: solid $surface;
        margin: 0 2;
        padding: 0 1;
        height: auto;
    }
    .field-row {
        layout: horizontal;
        height: 3;
    }
    .field-label {
        width: 16;
        padding: 1;
    }
    .field-input {
        width: 24;
    }
    .button-row {
        layout: horizontal;
        height: 3;
        padding: 0 0;
    }
    .button-row Button {
        margin: 0 1 0 0;
    }
    #control-log {
        margin: 0 2 0 2;
        border: solid $surface;
        min-height: 5;
    }
    """

    def compose(self) -> ComposeResult:
        yield Header()
        yield Static("Device Control", id="control-title")

        # -- Screen Element section --
        yield Static("Screen Element", classes="section-title")
        with Vertical(classes="section-box"):
            with Horizontal(classes="field-row"):
                yield Static("X Position:", classes="field-label")
                yield Input(placeholder="0", id="input-x", classes="field-input")
            with Horizontal(classes="field-row"):
                yield Static("Y Position:", classes="field-label")
                yield Input(placeholder="0", id="input-y", classes="field-input")
            with Horizontal(classes="field-row"):
                yield Static("Width:", classes="field-label")
                yield Input(placeholder="40", id="input-w", classes="field-input", value="40")
            with Horizontal(classes="field-row"):
                yield Static("Height:", classes="field-label")
                yield Input(placeholder="40", id="input-h", classes="field-input", value="40")
            with Horizontal(classes="field-row"):
                yield Static("Color:", classes="field-label")
                yield Input(placeholder="#RRGGBB or 0xNNNN", id="input-color", classes="field-input", value="#FFFFFF")
            with Horizontal(classes="field-row"):
                yield Static("Type:", classes="field-label")
                yield Input(placeholder="1=Pure Color", id="input-type", classes="field-input", value="1")
            with Horizontal(classes="field-row"):
                yield Static("Layer:", classes="field-label")
                yield Input(placeholder="0x0F", id="input-layer", classes="field-input", value="15")
            with Horizontal(classes="button-row"):
                yield Button("Apply", id="btn-apply-screen", variant="primary")
                yield Button("Apply + Refresh", id="btn-apply-refresh", variant="success")

        # -- Key Config section --
        yield Static("Key Config", classes="section-title")
        with Vertical(classes="section-box"):
            with Horizontal(classes="field-row"):
                yield Static("Arg0:", classes="field-label")
                yield Input(placeholder="0-255", id="input-arg0", classes="field-input")
            with Horizontal(classes="field-row"):
                yield Static("Key Index:", classes="field-label")
                yield Input(placeholder="0", id="input-key-index", classes="field-input", value="0")
            with Horizontal(classes="button-row"):
                yield Button("Apply", id="btn-set-arg0", variant="primary")
                yield Button("Apply + Save", id="btn-arg0-save", variant="success")

        # -- Actions section --
        yield Static("Actions", classes="section-title")
        with Horizontal(classes="button-row"):
            yield Static("", classes="field-label")  # spacer
            yield Button("Heartbeat", id="btn-heartbeat")
            yield Button("Refresh Display", id="btn-display-refresh")
            yield Button("Save", id="btn-save", variant="warning")
            yield Button("Send Raw...", id="btn-raw")

        yield RichLog(id="control-log", highlight=True, markup=True)
        yield Footer()

    def on_button_pressed(self, event: Button.Pressed) -> None:
        log = self.query_one("#control-log", RichLog)
        if self.app.device is None and event.button.id != "btn-raw":
            log.write("[yellow]Not connected. Go to Dashboard and press C.[/yellow]")
            return

        btn = event.button.id
        if btn == "btn-apply-screen":
            self._apply_screen(refresh=False)
        elif btn == "btn-apply-refresh":
            self._apply_screen(refresh=True)
        elif btn == "btn-set-arg0":
            self._set_arg0(save=False)
        elif btn == "btn-arg0-save":
            self._set_arg0(save=True)
        elif btn == "btn-save":
            self._save()
        elif btn == "btn-heartbeat":
            self._heartbeat()
        elif btn == "btn-display-refresh":
            self._display_refresh()
        elif btn == "btn-raw":
            self._send_raw()

    def _read_inputs(self) -> dict:
        """Read all input field values."""
        color_str = self.query_one("#input-color", Input).value or "#FFFFFF"
        if color_str.startswith("#"):
            from sayodevice.protocol import hex_color_to_565
            color = hex_color_to_565(color_str)
        else:
            color = int(color_str, 0)
        return {
            "x": int(self.query_one("#input-x", Input).value or "0"),
            "y": int(self.query_one("#input-y", Input).value or "0"),
            "w": int(self.query_one("#input-w", Input).value or "40"),
            "h": int(self.query_one("#input-h", Input).value or "40"),
            "color": color,
            "element_type": int(self.query_one("#input-type", Input).value or "1"),
            "layer": int(self.query_one("#input-layer", Input).value or "15"),
            "arg0": int(self.query_one("#input-arg0", Input).value or "0"),
            "key_index": int(self.query_one("#input-key-index", Input).value or "0"),
        }

    @work(thread=True)
    def _apply_screen(self, refresh: bool = False) -> None:
        log = self.query_one("#control-log", RichLog)
        dev = self.app.device
        try:
            v = self._read_inputs()
            resp = dev.set_screen_element(
                x=v["x"], y=v["y"], width=v["w"], height=v["h"],
                color=v["color"], element_type=v["element_type"],
                element_index=v["layer"], refresh=refresh,
            )
            msg = (
                f"SCREEN x={v['x']} y={v['y']} {v['w']}x{v['h']} "
                f"color=0x{v['color']:04X} type={v['element_type']} "
                f"layer={v['layer']}"
            )
            if refresh:
                msg += " + refresh"
            self.app.call_from_thread(log.write, f"[green]{msg}[/green]")
            if resp:
                self.app.call_from_thread(
                    log.write, f"[dim]Response: {bytes(resp)[:32].hex(' ')}[/dim]"
                )
        except Exception as e:
            self.app.call_from_thread(log.write, f"[red]Error:[/red] {e}")

    @work(thread=True)
    def _set_arg0(self, save: bool = False) -> None:
        log = self.query_one("#control-log", RichLog)
        dev = self.app.device
        try:
            v = self._read_inputs()
            resp = dev.set_key_config(
                arg0=v["arg0"], key_index=v["key_index"], save=save,
            )
            msg = f"KEY arg0={v['arg0']} key_index={v['key_index']}"
            if save:
                msg += " + saved"
            self.app.call_from_thread(log.write, f"[green]{msg}[/green]")
            if resp:
                self.app.call_from_thread(
                    log.write, f"[dim]Response: {bytes(resp)[:32].hex(' ')}[/dim]"
                )
        except Exception as e:
            self.app.call_from_thread(log.write, f"[red]Error:[/red] {e}")

    @work(thread=True)
    def _save(self) -> None:
        log = self.query_one("#control-log", RichLog)
        try:
            self.app.device.save()
            self.app.call_from_thread(log.write, "[green]Saved![/green]")
        except Exception as e:
            self.app.call_from_thread(log.write, f"[red]Error:[/red] {e}")

    @work(thread=True)
    def _heartbeat(self) -> None:
        log = self.query_one("#control-log", RichLog)
        try:
            self.app.device.send_heartbeat()
            self.app.call_from_thread(
                log.write, "[green]Heartbeat sent (SYS_INFO + SETTING)[/green]"
            )
        except Exception as e:
            self.app.call_from_thread(log.write, f"[red]Error:[/red] {e}")

    @work(thread=True)
    def _display_refresh(self) -> None:
        log = self.query_one("#control-log", RichLog)
        try:
            resp = self.app.device.refresh_display()
            self.app.call_from_thread(
                log.write, "[green]Display refresh sent[/green]"
            )
            if resp:
                self.app.call_from_thread(
                    log.write, f"[dim]Response: {bytes(resp)[:32].hex(' ')}[/dim]"
                )
        except Exception as e:
            self.app.call_from_thread(log.write, f"[red]Error:[/red] {e}")

    @work(thread=True)
    def _send_raw(self) -> None:
        log = self.query_one("#control-log", RichLog)
        self.app.call_from_thread(
            log.write,
            "[yellow]Raw command: use classic CLI[/yellow]\n"
            "[dim]  sayodevice --classic send-raw 0x00[/dim]\n"
            "[dim]  sayodevice --classic interactive[/dim]"
        )
